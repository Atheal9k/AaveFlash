...
const privateKey = '0x726d6cb3eb1278834ab67ab8d81d5c3ab3b5b6540ddef672818b458c3983bbc0';
...
const myContractAddress = '0xe77239bf9907d0907E4979F59a6307A2047C2275';
const myAbi = require('./aaveAbi.js');
...
const fromMyWallet = {
 ...
 const kek = async () => {
  ...
  let transferResult = await underlying.methods.transfer(
    myContractAddress,
    web3.utils.toWei("10000", "ether").toString() // 10 tokens to send to MyContract
  ).send(fromMyWallet);

  //await myContract.methods.deposit().send(fromMyWallet)

  let as = await underlying.methods.balanceOf(myContractAddress).call()
  const daiForReading = web3.utils.fromWei(as)
  console.log(daiForReading);

  console.log(`MyContract now has ${assetName} to supply to the Aave Protocol.`);

  const daiToSend = web3.utils.toWei("100", "ether").toString()
  ...
  let logs = await underlying.methods.allowance(myContractAddress,"0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D").call()
  console.log("allowed " + logs)
  //await myContract.methods.swapTokens().send(fromMyWallet)

  await myContract.methods.startFlashLoan(assetArray, amountArray).send(fromMyWallet)
  console.log("flashed")

  let ra = await myContract.methods.ethBalance().call()
  const raForReading = web3.utils.fromWei(ra)
  console.log(raForReading)

  let rasss = await myContract.methods.estimatedTokens().call()
  const rasssForReading = web3.utils.fromWei(rasss)
  console.log(rasssForReading)
